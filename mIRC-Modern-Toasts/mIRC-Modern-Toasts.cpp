...
void InitAppUserModelID(_In_ PCWSTR fallbackAppID);
HRESULT TryFindAppUserModelID();
HRESULT CreateShortcut(_In_ PCWSTR fallbackAppID);
HRESULT _CreateShortcut(_In_ PCWSTR path);
HRESULT _SetAppIDOnShortcut(_In_ PCWSTR fallbackAppID, _In_ PCWSTR path);

void __stdcall LoadDll(_Inout_ LOADINFO *loadInfo)
{
    InitAppUserModelID(L"mIRC.mIRC");
...
void InitAppUserModelID(_In_ PCWSTR fallbackAppID)
{
    ...
    {
        CreateShortcut(fallbackAppID);
        SHStrDup(fallbackAppID, &pszAppID);
    }
}
...
HRESULT CreateShortcut(_In_ PCWSTR fallbackAppID)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                hr = _SetAppIDOnShortcut(fallbackAppID, pszShortcutPath);
            }
        }
    }
    return hr;
...
HRESULT _SetAppIDOnShortcut(_In_ PCWSTR fallbackAppID, _In_ PCWSTR path)
{
    ...
    {
        PROPVARIANT pv;
        hr = InitPropVariantFromString(fallbackAppID, &pv);
        if (SUCCEEDED(hr))
        {
